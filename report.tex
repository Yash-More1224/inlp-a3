...
\begin{abstract}
This report presents the implementation and evaluation of neural sequence models for three tasks: (1) decrypting a substitution cipher using RNN and LSTM, (2) language modeling with SSM for word-level next-word prediction and BiLSTM for word-level masked language modeling, and (3) using language models to correct decryption errors in noisy cipher text. Task 3 evaluation could not be completed due to computational constraints. I implement all models from scratch in PyTorch and evaluate their performance using character accuracy, word accuracy, perplexity, BLEU, and ROUGE scores.
\end{abstract}

\section{Introduction}

The assignment involves building neural models to decrypt a cipher used to encrypt text. I am provided with:
\begin{itemize}
...
I implemented two sequence-to-sequence models from scratch (without using \texttt{nn.RNN} or \texttt{nn.LSTM}):
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{outputs/plots/task1_comparison.png}
\caption{Task 1: RNN vs LSTM Character Accuracy Comparison}
\label{fig:task1}
\end{figure}
...
I implemented two language modeling approaches with different granularities:
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{outputs/plots/task2_perplexity.png}
\caption{Task 2: SSM vs BiLSTM Perplexity Comparison}
\label{fig:task2}
\end{figure}
...
\begin{itemize}
    ...
    \item Both models show high perplexity, indicating word-level modeling difficulty
\end{itemize}
...
For this task, I combine the decryption model (LSTM from Task 1) with the language models (SSM or BiLSTM from Task 2) to correct errors introduced by noise in cipher\_01.txt through cipher\_04.txt.
...
\begin{itemize}
    \item Type: Word-level SSM (vocabulary $\sim$94K)
    \item Embedding dimension: 256
    \item State size: 512
    \item Dropout: 0.2
    \item Sequence length: 256
